...
it('separates a3 and c3 knight routes instead of merging their stems', () => {
...
it('gives all eight knight destinations separate starting lanes and right-angle bends', () => {
...
it('keeps ordinary piece arrows straight', () => {
  expect(moveArrowPath('e2', 'e4', false)).toBe('M450 650 L450 470');
});

for (const flipped of [false, true]) it(`centers visible arrow tips in every direction, flipped=${flipped}`, () => {
...
it('rotates all arrow coordinates for Black without merging knight lanes', () => {
